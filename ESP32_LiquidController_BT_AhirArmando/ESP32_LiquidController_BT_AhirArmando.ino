...
void sensor() {
  ...
  quantita_liquido = MAX_VOL - distance // quantita' = differenza tra distanza misurata e il totale del volume del serbatoio
  ...
  if(quantita > MAX_VOL) { //se QUANTITA' raggiunge il valore massimo impostato, allora il serbatoio è PIENO.
    SerialBT.println("WARNING: SERBATOIO PIENO");
    Serial.println("WARNING: SERBATOIO PIENO");
    buzzAlarm();
    } else if(quantita < SOGLIA){ //se QUANTITA' oltrepassa una SOGLIA minima impostata, allora il serbatoio è IN ESAURIMENTO
